...
bins = 10
saturation_boundaries = np.linspace(0,1,bins+1)

counts_empty = [{"to_bin_idx":i, "to_bin_boundaries":[saturation_boundaries[i],saturation_boundaries[i+1]],"positive":0,"negative":0} for i in range(bins)]
counts = [{"from_bin_idx":i, "from_bin_boundaries":[saturation_boundaries[i],saturation_boundaries[i+1]], "to_counts":deepcopy(counts_empty)} for i in range(bins)]
...
for i in range(df.shape[0]):
    ...
    anchor_saturation_idx = np.digitize(anchor_hsl[1],saturation_boundaries) - 1  #0 bin is for values below lowest boundary
    A_saturation_idx = np.digitize(A_hsl[1],saturation_boundaries) - 1  #0 bin is for values below lowest boundary
    B_saturation_idx = np.digitize(B_hsl[1],saturation_boundaries) - 1  #0 bin is for values below lowest boundary

    for from_idx in range(bins):
        # case 1: anchor is in bin
        if anchor_saturation_idx == from_idx:
            if preference == "A":
                # positive
                counts[from_idx]["to_counts"][A_saturation_idx]["positive"] += 1
                #negative
                counts[from_idx]["to_counts"][B_saturation_idx]["negative"] += 1

            elif preference == "B":
                # positive
                counts[from_idx]["to_counts"][B_saturation_idx]["positive"] += 1
                #negative
                counts[from_idx]["to_counts"][A_saturation_idx]["negative"] += 1
            
            else:
                raise ValueError("Preference should either be 'A' or 'B'.")
        
        else:
            if A_saturation_idx == from_idx:
                if preference == "A":
                    counts[from_idx]["to_counts"][anchor_saturation_idx]["positive"] += 1

                elif preference == "B":
                    counts[from_idx]["to_counts"][anchor_saturation_idx]["negative"] += 1

                else:
                    raise ValueError("Preference should either be 'A' or 'B'.")
                
            if B_saturation_idx == from_idx:
                if preference == "B":
                    counts[from_idx]["to_counts"][anchor_saturation_idx]["positive"] += 1
...
saturations = np.linspace(0.05, 0.95, 10)  # Generate hues from 0 to 1
colors = [convert_color(HSLColor(220,sat,0.5),sRGBColor).get_value_tuple() for sat in saturations] 
x = np.arange(10)
...
for i, color in enumerate(colors):
    ...
    ax.add_patch(rect)
plt.colorbar(p)
plt.savefig("saturation_analysis.pdf")
plt.show()
print("stop")